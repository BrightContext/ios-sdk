...
@interface FDSelectFeedViewController ()

@end

@implementation FDSelectFeedViewController
...
- (NSFetchedResultsController *)fetchedResultsController
{
    ...
    NSPredicate* validFeeds = [NSPredicate predicateWithFormat:@"type <> nil and name <> nil"];    
    NSArray* sortByName = [NSArray arrayWithObject:[NSSortDescriptor sortDescriptorWithKey:@"name"
                                                                                 ascending:YES]];
    FDAppDelegate* d = [[UIApplication sharedApplication] delegate];
    NSFetchedResultsController* frc = [d fetchedResultsControllerForEntity:@"Feed"
                                                                 batchSize:10
                                                                   section:nil
                                                                 cacheName:@"FeedSelection"
                                                           sortDescriptors:sortByName
                                                                 predicate:validFeeds];
    frc.delegate = self;
...
{
    NSManagedObject *object = [self.fetchedResultsController objectAtIndexPath:indexPath];
    if ([self.delegate respondsToSelector:@selector(feedSelector:didSelectFeed:)]) {
        [self.delegate feedSelector:self didSelectFeed:object];
    }
}
